...
SELECT COUNT(e.ManagerID) AS EmployeesWithManager
FROM Employees e



-- 8.Write a SQL query to find the number of all employees that have no manager.
SELECT COUNT(e.EmployeeID) AS EmployeesWithNoManager
FROM Employees e 
...
SELECT ROUND(AVG(e.Salary), 2) AS EmployessAvgSal, d.Name AS Departments
FROM Employees e 
JOIN Departments d
    ON e.DepartmentID = d.DepartmentID
GROUP BY d.Name
...
SELECT FORMAT(GETDATE(), 'dd.MM.yyyy HH:mm:ss:fff') AS DateFormat;
